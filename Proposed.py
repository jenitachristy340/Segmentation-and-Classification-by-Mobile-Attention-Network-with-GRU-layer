...
def worst_agents(X, M1, M2, dim, G_max, G_min, var_n_gases, var_n_types):
    ...
    X_sort, X_index = np.sort(X.fitness)
...
# Proposed -> update start from line no 113
def Proposed(X, objfunc, var_down, var_up, var_niter):
    var_n_gases = 35
    var_n_types = 5
    ...
    for t in range(var_niter):
        ...
        fitness = objfunc(X)
        r = fitness[t, :] / (min(fitness[t, :]) + max(fitness[t, :]))
        K = l1 * r
        P = l2 * r
        C = l3 * r
