...
def Model(X, Y, test_x, test_y, Optimizer):
    ...
    model.fit(X, Y, epochs=50, batch_size=32, validation_data=(test_x, test_y))
    pred = model.predict(test_x)
    return pred


def Model_CNN(train_data, train_target, test_data, test_target, Optimizer=None):
    ...
    pred = Model(Train_X, train_target, Test_X, test_target, Optimizer)
    pred[pred >= 0.5] = 1
    pred[pred < 0.5] = 0
